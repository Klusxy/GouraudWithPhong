...
GouraudShaderClass::GouraudShaderClass()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_inputLayout = 0;
...
bool GouraudShaderClass::init(ID3D11Device *device, HWND hwnd)
{
	return initShader(device, hwnd, L"gouraud.vs", L"gouraud.ps");
}

void GouraudShaderClass::stop()
{
	stopShader();
}

bool GouraudShaderClass::initShader(ID3D11Device *device, HWND hwnd, WCHAR *vsFilename, WCHAR *psFilename)
{
	HRESULT hr;
	ID3D10Blob *errorMsg;
...
void GouraudShaderClass::outputShaderErrorMsg(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
void GouraudShaderClass::stopShader()
{
	if (m_sampleState)
	{
...
bool GouraudShaderClass::setShaderParameters(ID3D11DeviceContext* deviceContext,
...
bool GouraudShaderClass::render(ID3D11DeviceContext *deviceContext, int indexCount, ID3D11ShaderResourceView *texture,
	XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
...
void GouraudShaderClass::renderShader(ID3D11DeviceContext *deviceContext, int indexCount, ID3D11ShaderResourceView *texture)
{
